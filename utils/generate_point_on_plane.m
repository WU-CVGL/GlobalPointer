function [plane_struct, lidar_pose_struct, B_sets, point_cloud_cell] = generate_point_on_plane(param)
    ...
    for lidar_pose_i = 1:param.lidar_pose_num
        ...
        if(param.randn_pose)
            ...
            noise_r = randn(1, 3);
            lidar_pose_struct(lidar_pose_i).R = R * axang2rotm([noise_r / norm(noise_r), mod(rand(1) * param.randn_pose_noise,1) * pi]);
        ...
        %     axang = axang(1:3) * axang(4);
        % 
        %     lidar_pose_struct(lidar_pose_i).R = axang2rotm([noise_r / norm(noise_r), norm(noise_r)]);
        %     lidar_pose_struct(lidar_pose_i).t = randn(1, 3);
        % else
        %     noise_r = randn(1, 3);
        %     lidar_pose_struct(lidar_pose_i).R = R * axang2rotm([noise_r / norm(noise_r), param.randn_pose_noise]);
        %     lidar_pose_struct(lidar_pose_i).t = t + param.randn_pose_noise * randn(1, 3);
        % end
    ...
    for plane_i = 1:param.plane_num
        ...
        plane_struct.normal_vector_gt_list{1, plane_i} = normalvec / norm(normalvec);
    ...
    point_cloud_cell = cell(param.lidar_pose_num, param.plane_num);
    ...
    for lidar_pose_i = 1:param.lidar_pose_num
        ...
        for plane_i = 1:param.plane_num
            ...
            point_cloud_global = point_cloud_global_gt + normrnd(0, param.randn_point_noise, size(point_cloud_global_gt));
            point_cloud_local = (point_cloud_global - t) * R;

            % outlier
            % point_cloud_local = point_cloud_local + randn(size(point_cloud_local));
