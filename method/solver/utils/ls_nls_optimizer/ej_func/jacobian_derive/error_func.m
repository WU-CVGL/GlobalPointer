...
function error = error_func(P1,P2,P3,nq,nx,ny,nz,rx,ry,rz,tx,ty,tz)
    ...
    t16 = rx.*rz.*t6.*t11;
    t17 = ry.*rz.*t6.*t11;
    error = nq+nx.*(tx+P3.*(t13-t16)+P1.*(t6.*t11.*(t3+t4)+1.0)-P2.*(t14+t15))+ny.*(ty+P1.*(t14-t15)+P2.*(t6.*t11.*(t2+t4)+1.0)-P3.*(t12+t17))+nz.*(tz+P2.*(t12-t17)+P3.*(t6.*t11.*(t2+t3)+1.0)-P1.*(t13+t16));
